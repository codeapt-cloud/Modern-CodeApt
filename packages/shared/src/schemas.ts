...
export const startMockInterviewResponseSchema =
  interviewCurrentResponseSchema.extend({
    ...
    /** Opening line the interviewer speaks before the first question (Step 35 F);
     *  uses the candidate's first name when available. */
...
export const submitInterviewAnswerResponseSchema = z.object({
  ...
  /** A neutral acknowledgement of the answer just given, spoken before the next
   *  question (Step 35 F). Never implies a score. "" when none. */
  acknowledgement: z.string().default(""),
  /** The closing line, present only when this answer ended the interview. */
